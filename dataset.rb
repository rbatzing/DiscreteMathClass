...
attr_accessor :table, :record
 ...
 #       "Medical: #{@table[:ithealth]}"
  ...
  def eastCoast?
    return(fieldValue(:region,@record[:region]).eql?("northeast"))
  end

  def westCoast?
    return(fieldValue(:region,@record[:region]).eql?("pacific"))
  end 

  def gradStudent?
    return(fieldValue(:education_group,@record[:education_group]).eql?("grad studies"))
  end

  def collegeGrad?
    return(fieldValue(:education_group,@record[:education_group]).eql?("college degree"))
  end

  def lowSalary?
    return(fieldValue(:income,@record[:income]).eql?("<$25K"))
  end

  def sales?
    return(fieldValue(:job_role,@record[:job_role]).eql?("sales"))
  end
...
d = Dataset.new("how-data-driven-is-your-organization.csv")
...
puts "Attribute counts: :age, :gender\n#{'=' * 32}"
puts "#{d.countColumn([:age,:gender,:income])}\n\n"
10.times {
